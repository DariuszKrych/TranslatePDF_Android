...
data class TranslationRecord(
    ...
    val timestamp: Long,        // When the translation finished, in epoch milliseconds.
    val outputPath: String      // Absolute path to the generated PDF in app private storage.
)

/**
 * Data access interface. Room generates an implementation with the actual SQL
 * work offloaded to a background safe coroutine dispatcher. Every method is
 * marked suspend so callers can simply await them.
 */
@Dao
...
/**
 * Room database wrapper. Declared abstract because Room generates the concrete
 * subclass that actually opens the underlying SQLite file ("translations.db").
 */
@Database(entities = [TranslationRecord::class], version = 1, exportSchema = false)
abstract class TranslationDatabase : RoomDatabase() {
    // Expose the DAO so the rest of the app can query and insert without
    // reaching into Room internals.
    abstract fun translationDao(): TranslationDao

    companion object {
        // Double checked locking singleton. @Volatile ensures other threads see
        // the assignment immediately and prevents two threads from each opening
        // their own database connection.
        @Volatile
        private var instance: TranslationDatabase? = null

        /**
         * Return the process wide database instance, creating it on first use.
         * Using the application context avoids leaking any Activity or Fragment
         * context into Room.
         */
        fun get(context: Context): TranslationDatabase {
            return instance ?: synchronized(this) {
                instance ?: Room.databaseBuilder(
                    ...
                    "translations.db"   // Physical SQLite file stored in the app private data dir.
