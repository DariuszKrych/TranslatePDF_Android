...
// Every "on…" color is the foreground drawn on top of its matching container color.
private val DarkColorScheme = darkColorScheme(
    primary = White,                         // Main accent — buttons, toolbar icons.
    onPrimary = Black,                       // Text/icons drawn on `primary` surfaces.
    primaryContainer = DarkGrey,             // Filled containers derived from primary.
    onPrimaryContainer = White,              // Content on `primaryContainer`.
    secondary = White,                       // Secondary accent (unused heavily, kept consistent).
    onSecondary = Black,
    ...
    surface = Black,                         // Cards / sheets background.
    onSurface = White,                       // Text/icons drawn on surfaces.
    ...
    inverseSurface = White,                  // Inverted surface (snackbars etc.).
    inverseOnSurface = Black,
    inversePrimary = Black,
    outline = White,                         // Border color for outlined buttons/text fields.
    outlineVariant = DarkGrey,               // Subtler outline for dividers.
    surfaceBright = DarkGrey,                // Elevated surface tones — all mapped to DarkGrey/Black
...
// Mirrors the dark scheme with black/white swapped for a high-contrast look.
...
/**
 * Top-level Compose theme wrapper. Every Compose UI in the app is hosted inside this
 * so it picks up the correct light/dark palette and shared typography.
 *
 * Reads [ThemeState.isDark] — a Compose-observable flag updated by the activity and
 * the settings screen — to decide which scheme to apply.
 */
@Composable
fun TranslatePDFTheme(
    ...
    // Apply scheme + typography to the subtree so any descendant composable can read them.
    MaterialTheme(
